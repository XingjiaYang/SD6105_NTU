...
| 文件 | 行数 | 列数 | 年份 | 生成脚本 |
|------|------|------|------|----------|
| `ipeds_enrollment.csv` | 43,276 | 61 | 2017–2023 | `fetch_ic_adm_ef.py` |
| `ipeds_admissions.csv` | 14,057 | 24 | 2017–2023 | `fetch_ic_adm_ef.py` |
| `ipeds_ic.csv` | 50,505 | 26 | 2017–2024 | `fetch_ic_adm_ef.py` |
| `ipeds_institutions.csv` | — | — | 2017–2024 | `fetch_ipeds.py` |
| `ipeds_completions.csv` | — | — | 2017–2024 | `fetch_ipeds.py` |
| `ipeds_financial_aid.csv` | — | — | 2017–2023 | `fetch_ipeds.py` / `fetch_sfa_f.py` |
| `ipeds_finance.csv` | — | — | 2017–2023 | `fetch_ipeds.py` / `fetch_sfa_f.py` |
| `ipeds_panel_2017_2023.csv` | 51,789 | 75 | 2017–2024 | `build_ipeds_panel.py` |

---

## 未纳入的参考列说明

以下参考文件（`IPEDS_data.csv`）中的列因数据来源未下载或结构过于复杂，**未予纳入**：

| 参考列 | 原因 |
|--------|------|
| 本科毕业率（4/5/6 年） | GR 模块 grtype/chrtstat 编码结构复杂，需专项数据字典解析，暂跳过 |
| 历年学费（2010-11 至 2013-14） | IC_AY 仅记录当年学费；已改用当年 CHG*AY3 列提供 |
| 捐赠资产/FTE | 需 EFIA 模块计算 FTE；ipeds_finance.csv 提供年末捐赠总额 |
| "Estimated enrollment"系列 | 来自 EFIA 模块（12 个月在校人数），非 EF 秋季调查 |
| SAT Writing 25th/75th | College Board 于 2016 年取消 SAT 写作部分 |
...
| `offer_first_prof_cert` | 是否提供第一职业证书 | LEVEL12 | 0=否, 1=是（遗留类别，几乎全为 0） |
| `offer_doctoral_research` | 是否提供研究型博士 | LEVEL17 | 0=否, 1=是 |
| `offer_doctoral_professional` | 是否提供专业实践型博士 | LEVEL18 | 0=否, 1=是 |
| `offer_doctoral_other` | 是否提供其他类型博士 | LEVEL19 | 0=否, 1=是 |
| `open_admissions` | 开放招生政策 | OPENADMP | 1=实行开放招生, 0=否 |
| `room_available` | 提供校内住宿 | ROOM | 1=是, 0=否 |
| `board_available` | 提供校内餐饮 | BOARD | 1=是, 0=否 |
...
| `enrolled_ft` | 全日制入学人数 | ENRLFT |
| `enrolled_pt` | 兼读制入学人数 | ENRLPT |
| `admission_rate` | 录取率（%）= admissions / applicants × 100 | 计算列 |
| `enrollment_yield` | 入学率（%）= enrolled / admissions × 100 | 计算列 |
| `pct_submit_sat` | 提交 SAT 成绩的新生比例（%） | SATPCT |
| `pct_submit_act` | 提交 ACT 成绩的新生比例（%） | ACTPCT |
| `sat_read_25th` | SAT 阅读/证据写作 25th 百分位 | SATVR25 |
| `sat_read_50th` | SAT 阅读/证据写作 50th 百分位 | SATVR50 |
| `sat_read_75th` | SAT 阅读/证据写作 75th 百分位 | SATVR75 |
| `sat_math_25th` | SAT 数学 25th 百分位 | SATMT25 |
| `sat_math_50th` | SAT 数学 50th 百分位 | SATMT50 |
| `sat_math_75th` | SAT 数学 75th 百分位 | SATMT75 |
| `act_composite_25th` | ACT 综合 25th 百分位 | ACTCM25 |
| `act_composite_50th` | ACT 综合 50th 百分位 | ACTCM50 |
| `act_composite_75th` | ACT 综合 75th 百分位 | ACTCM75 |
| `act_english_25th` | ACT 英语 25th 百分位 | ACTEN25 |
| `act_english_75th` | ACT 英语 75th 百分位 | ACTEN75 |
| `act_math_25th` | ACT 数学 25th 百分位 | ACTMT25 |
| `act_math_75th` | ACT 数学 75th 百分位 | ACTMT75 |
...
| 列名 | 说明 | 原始变量 |
|------|------|---------|
| `retention_rate_ft` | 全日制首次入读新生保留率（%）：上一年秋季入读全日制新生在本年秋季继续就读的比例 | RET_PCF |
| `retention_rate_pt` | 兼读制首次入读新生保留率（%）| RET_PCP |
| `ret_ft_cohort` | 全日制保留率计算分母（调整后队列人数）| RRFTCTA |
| `ret_ft_retained` | 全日制保留率计算分子（留存人数）| RET_NMF |
| `ret_pt_cohort` | 兼读制保留率计算分母（调整后队列人数）| RRPTCTA |
| `ret_pt_retained` | 兼读制保留率计算分子（留存人数）| RET_NMP |
| `student_faculty_ratio` | 师生比（学生数 : 教师数）| STUFACR |
| `gr_cohort` | 毕业率队列人数（入读学士学位课程的首次全日制新生）| GRCOHRT |
| `ug_entering` | 本科入学新生总数 | UGENTERN |

**保留率验证**：retention_rate_ft ≈ ret_ft_retained / ret_ft_cohort × 100（已对 2017–2023 年全部验证，偏差 ≤1pp）。
...
| 列名 | 说明 | 备注 |
|------|------|------|
| `ftug_instate_n` | 来自本州的首次本科新生人数 | 匹配 fips_state |
| `ftug_instate_pct` | 本州来源比例（%） | |
| `ftug_outstate_n` | 来自外州的首次本科新生人数 | = 合计 - 本州 - 境外 - 未知 |
| `ftug_outstate_pct` | 外州来源比例（%） | |
| `ftug_foreign_n` | 来自境外的首次本科新生人数 | EFCSTATE=57 |
| `ftug_foreign_pct` | 境外来源比例（%） | |
| `ftug_unknown_n` | 来源地未知的首次本科新生人数 | EFCSTATE=98 |
| `ftug_unknown_pct` | 来源地未知比例（%） | |
...
| 列名 | 说明 | 原始变量 | 备注 |
|------|------|---------|------|
| `tuition_fees_instate` | 在州学生学费+必要费用合计（美元） | CHG2AY3 | 在州全日制本科生 |
| `tuition_fees_outstate` | 外州学生学费+必要费用合计（美元） | CHG3AY3 | 外州全日制本科生 |
| `tuition_fees_district` | 学区内学生学费+必要费用（美元） | CHG1AY3 | 仅公立两年制机构适用 |
| `room_charge` | 住校住宿+餐饮费（美元） | CHG5AY3 | 住校（on-campus）room & board |
| `books_supplies` | 书本及学习材料费（美元） | CHG4AY3 | 预估费用 |
| `total_price_instate_oncampus` | 在州学生住校总费用（美元） | CHG2+4+5+6AY3 | 学费+书本+住宿+其他 |
| `total_price_outstate_oncampus` | 外州学生住校总费用（美元） | CHG3+4+5+6AY3 | 学费+书本+住宿+其他 |
...
6. **地理来源比例**: EF-C 中境外学生来源以 EFCSTATE=57 标识，已单独拆为 `ftug_foreign_n/pct`；外州 = 合计 - 本州 - 境外 - 未知。
